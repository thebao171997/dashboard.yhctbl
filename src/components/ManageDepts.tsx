    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Tên khoa</label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Loại khoa</label>
              <select
                value={type}
          ...
          {type === DEPT_TYPES.INPATIENT && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Số giường kế hoạch (Dùng để tính CSSDGB)</label>
              <input
                type="number"
                min="1"
                value={plannedBeds}
                onChange={e => setPlannedBeds(e.target.value)}
                className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 max-w-xs"
                placeholder="VD: 50"
                required
              />
            </div>
          ...
          {type !== DEPT_TYPES.FUNCTIONAL && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-3">Các chỉ tiêu theo dõi của khoa</label>
                        ...
                        return (
                          ...
                          >
                            <div className={clsx(
                              ...
                              isSelected ? "bg-cyan-500 text-white" : "bg-slate-200"
                            )}>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-3">Nhân lực</label>
            <div className="space-y-6">
              {PERSONNEL_GROUPS.map(group => (
                <div key={group.title} className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {group.keys.map(key => (
                      <div key={key}>
                        <label className="block text-xs font-medium text-slate-500 mb-1">{PERSONNEL_LABELS[key]}</label>
                        <input
                          type="number"
                          min="0"
                          value={personnel[key] || ''}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100">
          <h3 className="text-xl font-bold text-slate-800">Danh sách khoa đã tạo</h3>
        </div>
        {loading ? (
          <div className="p-8 text-center text-slate-500">Đang tải...</div>
        ) : departments.length === 0 ? (
          <div className="p-8 text-center text-slate-500">Chưa có khoa nào được tạo.</div>
        ) : (
